...
- `AssetSpecModel`: This contains attributes that are common to all assets, such as the key, description, tags, and dependencies.
...
Because the argument names in the schema match the names of the arguments in the `ShellCommandComponent` class, the `load` method will automatically populate the class with the values from the schema, and will automatically resolve the `AssetSpecModel`s into `AssetSpec` objects.
